...
- **T4.5 `[x]` Image niceties** — on upload, record width/height (`getimagesize()`), emit them on `<img>` in rendered posts (CLS fix) + `loading="lazy"` everywhere images render (public, admin grid, widget). *Accept:* additive `media` columns via `migrate_schema()`; old rows tolerate NULL dims.
- **T4.6 `[x]` 404 + error pages match the design** — styled not-found page with search box and recent posts. *Accept:* correct 404 status code preserved.
